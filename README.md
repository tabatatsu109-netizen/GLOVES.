...
| `/オリジナルプリント/`（`オリジナルプリント/index.html`） | ORIGINAL PRINT LP + 見積もりシミュレーター（Tシャツ・パーカー） |
...
## 11. ORIGINAL PRINT（見積もりシミュレーター）

`オリジナルプリント/index.html` は、グローブとは別ラインの「Tシャツ・パーカーへのオリジナルプリント」を紹介する LP です。
`base.css` / `lp.css` / `designer.css` をそのまま再利用しているため、見た目はグローブのLPと共通です。

```
オリジナルプリント/
  index.html            ORIGINAL PRINT LP + 見積もりシミュレーター
assets/
  css/print.css         見積もりシミュレーター専用のスタイル
  js/print.js           ★ 商品・価格データ + 見積もりロジック(ここだけ触れば商品/単価を変更できる)
  images/print-*.jpg    商品写真(実写 or ダミー)
```

### 価格・商品の変更

`assets/js/print.js` の先頭にある `PRICING`（数量帯・複数箇所割引・プリント単価）と
`PRODUCTS`（商品・カラー・プリント位置）を編集するだけです。ビルドは不要です。

現在の単価は `価格表/価格表new.xlsx`（2024-06-24時点）の「オリジナルウェア(ガーメント)」シートに基づく実データです。
`basePrice`（本体価格）は仕入原価からの概算のため、実際の店頭価格に合わせて調整してください。

写真が無い商品・カラー（現状はパーカー）は自動で簡易イラスト表示になります。
写真を用意できたら、該当する `colors[].img` に `front` / `back` のパスを追加するだけで実写に切り替わります。

### 見積もり依頼の送信

「この内容で問い合わせる」は `core.js` の `NOF.submitOrder()` をそのまま使っています。
`NOF_CONFIG.orderEndpoint` が未設定の間はブラウザの `localStorage`（キー `nof.orders`）にデモ保存され、
設定すれば同じ処理で実際のAPIにPOSTされます（team.html の注文と同じ仕組みです）。

### PNG保存

「この画像を保存」は `designer.js` と同じ考え方で、Canvas 上に商品写真＋アップロード画像を合成してPNG書き出しします。
